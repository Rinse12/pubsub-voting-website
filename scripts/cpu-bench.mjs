// Steady-state CPU + memory benchmark: drive the REAL site in real Chromium and attribute
// CPU seconds to a window of wall clock, so "does this dependency bump stop the laptop fans"
// is a measurement, not an argument.
//
// The sibling harness (coldstart-bench.mjs) answers "how long until the page is usable".
// This one answers the opposite question — what the tab costs once it IS usable and is just
// sitting there, which is the cost a user actually lives with. The two windows are reported
// separately because they have different causes: cold is boot + 63 joins + the root-fetch
// burst, steady is whatever the page never stops doing.
//
...
//
// Env: ROUNDS (3), SETTLE_MS (60000), WINDOW_MS (120000), SAMPLE_MS (2000),
//      HEALTH_AT_MS (20000), QUERY, LABEL, HEADED=1, SKIP_BUILD=1, OUT=<file.json>,
//      SITE_ROOT (this repo), PLAYWRIGHT_FROM, CHROME_PATH, PROFILE_BASE
